...
{
    [Authorize]
    [RoutePrefix("api/planning-production")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class PlanningProductionMngController : ApiController
    {
        private string moduleCode = "PlanningProductionMng";
        private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.PlanningProductionMng.Executor, Module.PlanningProductionMng");
        ...
        /// <summary>
        /// create planning production by copy from bom
        /// </summary>
        /// <param name="bomID"></param>
        /// <returns></returns>
        [HttpPost]
        [Route("create-planning-production-from-bom")]
        public IHttpActionResult CreatePlanningProductionFromBOM(int? bomID)
        {
            Hashtable param = new Hashtable();
            param["bomID"] = bomID;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "create-planning-production-from-bom", param, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
       ...
       /// <returns></returns>
        [HttpPost]
        [Route("get-production-team")]
        public IHttpActionResult GetProductionTeam(string param)
        {
            Hashtable filter = new Hashtable();
            filter["searchQuery"] = param;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "get-production-team", filter, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("get-grant-chart")]
        public IHttpActionResult GetGrantChart(int planningProductionID)
        {
            Hashtable param = new Hashtable();
            param["planningProductionID"] = planningProductionID;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "get-grant-chart", param, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
