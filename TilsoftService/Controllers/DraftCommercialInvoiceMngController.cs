...
{
    [Authorize]
    [RoutePrefix("api/draftcommercialinvoicemng")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class DraftCommercialInvoiceMngController : ApiController
    {
		private string moduleCode = "DraftCommercialInvoiceMng";
		private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.DraftCommercialInvoiceMng.Executor, Module.DraftCommercialInvoiceMng");
        ...
        [HttpPost]
        [Route("getclient")]
        public IHttpActionResult GetInitData(string param)
        {
        ...
        [HttpPost]
        [Route("getsaleorder")]
        public IHttpActionResult GetSaleOrder(int clientID, int checkSelect, string season, string proformaInvoiceNo, string articleCode)
        {
            Hashtable input = new Hashtable();
            input["clientID"] = clientID;
            input["checkSelect"] = checkSelect;
            input["season"] = season;
            input["proformaInvoiceNo"] = proformaInvoiceNo;
            input["articleCode"] = articleCode;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "getsaleorder", input, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
        ...
        public IHttpActionResult GetOverView(int id)
        {
            ...
            Hashtable input = new Hashtable();
            input["id"] = id;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "getOverView", input, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("getDraftInvoicePrintoutData")]
        public IHttpActionResult GetDraftInvoicePrintoutData(int id)
        {
            // authentication
            if (!fwBll.CanPerformAction(ControllerContext.GetAuthUserId(), moduleCode, Library.DTO.ModuleAction.CanRead))
            {
            ...
            Hashtable input = new Hashtable();
            input["id"] = id;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "getDraftInvoicePrintoutData", input, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
    }
}
