...
namespace TilsoftService.Controllers
{
    ...
    [RoutePrefix("api/qcreport")]
    public class QCReportController : ApiController
    {
        private string moduleCode = "QCReport";
        private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.QCReport.Executor, Module.QCReport");
        ...
        [HttpPost]
        [Route("get")]
        public IHttpActionResult Get(int id, int f)
        {
            // authentication
            if (!fwBll.CanPerformAction(ControllerContext.GetAuthUserId(), moduleCode, Library.DTO.ModuleAction.CanRead))
            {
        ...
        [HttpPost]
        [Route("getinitsearchfilter")]
        public IHttpActionResult GetInitSearchFilterData()
        {
            Library.DTO.Notification notification;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "getinitsearchfilter", null, out notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("getinit")]
        public IHttpActionResult getInit(DTO.Search searchInput)
        {
            // authentication
            if (!fwBll.CanPerformAction(ControllerContext.GetAuthUserId(), moduleCode, Library.DTO.ModuleAction.CanRead))
            {
            ...
            int totalRows = 0;
            inputs = searchInput.Filters;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "getinitdatawithfilter", inputs, out notification);
            return Ok(new Library.DTO.ReturnData<Object>() { Data = data, Message = notification, TotalRows = totalRows });
        }
