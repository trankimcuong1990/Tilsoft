...
namespace TilsoftService.Controllers
{
    ...
    [RoutePrefix("api/productionItem")]
    public class ProductionItemController : ApiController
    {
        private string moduleCode = "ProductionItem";
        private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.ProductionItem.Executor, Module.ProductionItem");
        ...
        public IHttpActionResult ExportExcel (int id)
        {
            ...
            System.Collections.Hashtable input = new System.Collections.Hashtable();
            input["ID"] = id;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "exportexcel", input, out notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("exportexcellistproductionitem")]
        public IHttpActionResult ExportExcel(DTO.Search searchInput)
        {
            ...
            Library.DTO.Notification notification;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "exportexcellistproductionitem", searchInput.Filters, out notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
    }
}
