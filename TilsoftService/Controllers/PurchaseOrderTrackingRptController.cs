...
{
    [Authorize]
    [RoutePrefix("api/purchaseOrderTrackingRpt")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class PurchaseOrderTrackingRptController : ApiController
    {
		private string moduleCode = "PurchaseOrderTrackingRpt";
		private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.PurchaseOrderTrackingRpt.Executor, Module.PurchaseOrderTrackingRpt");
        ...
        [HttpPost]
        [Route("getclient")]
        public IHttpActionResult GetClientWithFilter(string param)
        {
            if (!fwBll.CanPerformAction(ControllerContext.GetAuthUserId(), moduleCode, Library.DTO.ModuleAction.CanRead))
            {
                return InternalServerError(new Exception(Properties.Resources.NOT_AUTHORIZED));
            ...
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "GetClientWithFilter", filters, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("getworkorder")]
        public IHttpActionResult GetWorkOrderWithFilter(string param, string param2)
        {
            if (!fwBll.CanPerformAction(ControllerContext.GetAuthUserId(), moduleCode, Library.DTO.ModuleAction.CanRead))
            {
                return InternalServerError(new Exception(Properties.Resources.NOT_AUTHORIZED));
            ...
            Hashtable filters = new Hashtable();
            filters["searchString"] = param;
            filters["clientID"] = param2;

            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "GetWorkOrderWithFilter", filters, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
