...
{
    [Authorize]
    [RoutePrefix("api/productionstatistics")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class ProductionStatisticsMngController : ApiController
    {
		private string moduleCode = "ProductionStatisticsMng";
		private Library.Base.IExecutor2 executor = Library.Helper.GetDynamicObject2("Module.ProductionStatisticsMng.Executor, Module.ProductionStatisticsMng");
        ...
        //
        [HttpPost]
        [Route("get-planning-production-team")]
        public IHttpActionResult GetPlanningProductionTeam(int workCenterID, string producedDate)
        {
            Hashtable input = new Hashtable();
            input["workCenterID"] = workCenterID;
            input["producedDate"] = producedDate;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "get-planning-production-team", input, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
