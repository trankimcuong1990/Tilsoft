namespace TilsoftService.Controllers
{
    ...
    [Authorize]
    [RoutePrefix("api/outsourceRpt")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class OutsourceRptController : ApiController
    {
        private readonly string moduleCode = "OutsourceRpt";
        private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.OutsourceRpt.Executor, Module.OutsourceRpt");
        ...
        [HttpPost]
        [Route("getDocumentNote")]
        public IHttpActionResult GetDocumentNote(DTO.Search searchInput)
        {
            // authentication
            if (!fwBll.CanPerformAction(ControllerContext.GetAuthUserId(), moduleCode, Library.DTO.ModuleAction.CanRead))
            {
            ...
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "GetDocumentNote", searchInput.Filters, out notification);

            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("exportOutsourceReport")]
