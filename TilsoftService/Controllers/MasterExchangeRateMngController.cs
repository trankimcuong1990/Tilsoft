...
{
    [Authorize]
    [RoutePrefix("api/MasterExchangeRateMng")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class MasterExchangeRateMngController : ApiController
    {
		private string moduleCode = "MasterExchangeRateMng";
		private Library.Base.IExecutor executor = Library.Helper.GetDynamicObject("Module.MasterExchangeRateMng.Executor, Module.MasterExchangeRateMng");
        ...
        [HttpPost]
        [Route("updateindexdata")]
        public IHttpActionResult UpdateSellingPrice(object dataindex)
        {
            // authentication
            ...
            input["data"] = dataindex;
            var data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "updateindex", input, out notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
    }
}
