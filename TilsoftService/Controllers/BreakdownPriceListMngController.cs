...
{
    [Authorize]
    [RoutePrefix("api/breakdown-price-list")]
    [TilsoftService.Lib.LogFilterAtrribute]
    public class BreakdownPriceListMngController : ApiController
    {
        private string moduleCode = "BreakdownPriceListMng";
        private Library.Base.IExecutor2 executor = Library.Helper.GetDynamicObject2("Module.BreakdownPriceListMng.Executor, Module.BreakdownPriceListMng");
        ...
        [HttpPost]
        [Route("update-price")]
        public IHttpActionResult UpdatePrice(object priceListTable)
        {
            Hashtable input = new Hashtable();
            input["priceListTable"] = priceListTable;
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "update-price", input, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }

        [HttpPost]
        [Route("add-production-item-price")]
        public IHttpActionResult AddProductionItemPrice()
        {
            object data = executor.CustomFunction(ControllerContext.GetAuthUserId(), "add-production-item-price", null, out Library.DTO.Notification notification);
            return Ok(new Library.DTO.ReturnData<object>() { Data = data, Message = notification });
        }
