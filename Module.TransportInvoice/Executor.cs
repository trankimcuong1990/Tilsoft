        ...
        public object CustomFunction(int userId, string identifier, System.Collections.Hashtable filters, out Library.DTO.Notification notification)
        {
            ...
            switch (identifier.Trim())
            {
                case "GetData":
                    ...
                    int? bookingID = Convert.ToInt32(filters["bookingID"]);
                    return bll.GetData(userId, id, bookingID, out notification);

                case "GetBooking":
                    return bll.GetBooking(userId, filters, out notification);
                case "SetInvoiceStatus":
                    int statusID = Convert.ToInt32(filters["statusID"]);
                    int transportInvoiceID = Convert.ToInt32(filters["transportInvoiceID"]);
                    return bll.SetInvoiceStatus(userId, statusID, transportInvoiceID, out notification);
                default:
