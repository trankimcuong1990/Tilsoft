...
namespace Module.ProductionStatisticsMng
{
    public class Executor : Library.Base.IExecutor2
    {
        ...
        public object CustomFunction(int userId, string identifier, System.Collections.Hashtable input, out Library.DTO.Notification notification)
        {
            ...
            {
                case "get-planning-production-team":
                    int workCenterID = Convert.ToInt32(input["workCenterID"]);
                    string producedDate = input["producedDate"].ToString();
                    return bll.GetPlanningProductionTeam(userId, workCenterID, producedDate, out notification);
                default:
                    break;
            }
