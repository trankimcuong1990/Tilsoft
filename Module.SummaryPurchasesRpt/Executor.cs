...
namespace Module.SummaryPurchasesRpt
{
    public class Executor : Library.Base.IExecutor
    {
        ...
        public object CustomFunction(int userId, string identifier, Hashtable input, out Notification notification)
        {
            ...
            string temp = "";
            string productionItemUD = (input.ContainsKey("productionItemUD") && input["productionItemUD"] != null && !string.IsNullOrEmpty(input["productionItemUD"].ToString()) ? Convert.ToString(input["productionItemUD"]) : temp );
            ...
            string factoryRawMaterialFullName = (input.ContainsKey("factoryRawMaterialFullName") && input["factoryRawMaterialFullName"] != null && !string.IsNullOrEmpty(input["factoryRawMaterialFullName"].ToString()) ? Convert.ToString(input["factoryRawMaterialFullName"]) : temp);
            switch (identifier.Trim())
            {
                case "getexcelreport":
                    return Bll.GetExcelReportData(userId, factoryRawMaterialID, productionItemUD, startDate, endDate, factoryRawMaterialFullName, out notification);
