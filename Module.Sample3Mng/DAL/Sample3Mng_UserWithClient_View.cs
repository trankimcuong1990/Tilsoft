//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Module.Sample3Mng.DAL
{
    using System;
    using System.Collections.Generic;
    
    public partial class Sample3Mng_UserWithClient_View
    {
        public int ClientID { get; set; }
        public Nullable<int> SaleID { get; set; }
        public Nullable<int> Sale2ID { get; set; }
        public Nullable<int> SaleVNID { get; set; }
        public string SaleNM { get; set; }
        public string SaleCompNM { get; set; }
        public string Sale2NM { get; set; }
        public string SaleComp2NM { get; set; }
        public string SaleVNNM { get; set; }
        public string SaleCompVNNM { get; set; }
    }
}
