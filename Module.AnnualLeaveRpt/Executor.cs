        ...
        public object CustomFunction(int userId, string identifier, System.Collections.Hashtable input, out Library.DTO.Notification notification)
        {
            ...
            switch (identifier.ToLower())
            {
                case "getdetail":
                    ...
                    {
                        notification = new Library.DTO.Notification() { Type = Library.DTO.NotificationType.Error, Message = "Unknow employeeID" };
                        return null;
                    }
                    if (!input.ContainsKey("affectedYear"))
                    {
                        notification = new Library.DTO.Notification() { Type = Library.DTO.NotificationType.Error, Message = "Unknow affectedYear" };
                        return null;
                    }
                    int employeeID = Convert.ToInt32(input["employeeID"]);
                    int affectedYear = Convert.ToInt32(input["affectedYear"]);
                    return bll.GetDetails(employeeID, affectedYear, out notification);
                case "getdetailtotal":
                    if (!input.ContainsKey("employeeID"))
                    {
                        notification = new Library.DTO.Notification() { Type = Library.DTO.NotificationType.Error, Message = "Unknow employeeID" };
                    ...
                    return bll.GetDetailTotal(Convert.ToInt32(input["employeeID"]), Convert.ToInt32(input["affectedYear"]), Convert.ToInt32(input["type"]), out notification);
