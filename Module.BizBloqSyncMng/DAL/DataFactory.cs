...
namespace Module.BizBloqSyncMng.DAL
{
    internal class DataFactory : Library.Base.DataFactory<DTO.SearchFormData, DTO.EditFormData>
    {
        ...
        private BizBloqSyncMngEntities CreateContext()
        {
            return new BizBloqSyncMngEntities(Library.Helper.CreateEntityConnectionString("DAL.BizBloqSyncMngModel"));
        }
        ...
        public bool UpdateData(int userId, object jsonData, out Library.DTO.Notification notification)
        {
            notification = new Library.DTO.Notification() { Type = Library.DTO.NotificationType.Success };

            try
            {
                Newtonsoft.Json.Linq.JObject jObject = (Newtonsoft.Json.Linq.JObject)jsonData;
                List<DTO.StockDTO> data = jObject["stock_data"].ToObject<List<DTO.StockDTO>>();
                if (data != null && data.Count() > 0)
                {
                    using (BizBloqSyncMngEntities context = CreateContext())
                    {
                        foreach (DTO.StockDTO item in data)
                        {
                            WexItem dbItem = context.WexItem.FirstOrDefault(o => o.ArticleCode == item.articleCode);
                            if (dbItem == null)
                            {
                                context.WexItem.Add(dbItem);
                            }
                            dbItem.ArticleCode = item.articleCode;
                            dbItem.SubEANCode = item.subEANCode;
                            dbItem.StockQnt = item.availableQnt;
                            dbItem.TotalStockQnt = item.stockQnt;
                        }
                        context.SaveChanges();
                    }
                }
                notification.Message = "Stock data updated successfully!";
                return true;
            }
            catch (Exception ex)
            {
