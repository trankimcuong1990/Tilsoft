...
namespace Module.PlanningProductionMng
{
    public class Executor : Library.Base.IExecutor
    {
        ...
        public object CustomFunction(int userId, string identifier, System.Collections.Hashtable param, out Library.DTO.Notification notification)
        {
            ...
            {
                case "create-planning-production-from-bom":
                    int bomID = Convert.ToInt32(param["bomID"]);                    
                    return bll.CreatePlanningProductionFromBOM(bomID, userId, out notification);

                case "get-production-team":
                    string searchQuery = param["searchQuery"].ToString();
                    return bll.GetProductionTeam(searchQuery, out notification);

                case "get-grant-chart":
                    int planningProductionID = Convert.ToInt32(param["planningProductionID"]);
                    return bll.GetGrantChart(planningProductionID, out notification);
            }
