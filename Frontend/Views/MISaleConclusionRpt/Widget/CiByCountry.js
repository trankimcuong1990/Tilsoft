...
angular.module('tilsoftApp').controller('MiSaleConclusionRpt_CiByCountryController', ['$scope', '$rootScope', '$timeout', '$filter', function ($scope, $rootScope, $timeout, $filter) {
    $scope.miSaleConclusionRpt = {};
    //
    // property
    //
    $scope.miSaleConclusionRpt.data = null;
    $scope.miSaleConclusionRpt.param = miSaleConclusionRpt_CiByCountry_Context.param;
                ...
                function (data) {
                    ...
                    angular.forEach($scope.miSaleConclusionRpt.data, function (item) {
                        if (item.totalAmount !== 0) {
                            params.data.push({ name: item.clientCountryNM, y: item.totalAmount });
                        }                        
                    });
                    jQuery('#miSaleConclusionRpt_CiConfirmedByCountry').highcharts({
                        chart: {
        ...
        getData: function (season, successCallBack, errorCallBack) {
            $.ajax({
                ...
                headers: {
                    ...
                    'Authorization': 'Bearer ' + miSaleConclusionRpt_CiByCountry_Context.token
                },
                type: "POST",
                dataType: 'json',
                url: miSaleConclusionRpt_CiByCountry_Context.serviceUrl + 'get-ci-confirmed-by-country?season=' + season + '&saleID=-1',
                beforeSend: function () {
