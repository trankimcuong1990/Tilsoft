...
angular.module('tilsoftApp').controller('PiConfirmedByCountryController', ['$scope', '$rootScope', '$timeout', '$filter', function ($scope, $rootScope, $timeout, $filter) {
    $scope.miSaleConclusionRpt = {};
    //
    // property
    //
    $scope.miSaleConclusionRpt.data = null;
    $scope.miSaleConclusionRpt.param = miSaleConclusionRpt_PiConfirmedByCountry_Context.param;
    ...
    $scope.miSaleConclusionRpt.sortValue = {
        colName: '-totalConfirmedProformaAmount'
    };
            ...
            $scope.miSaleConclusionRpt.method.getData(
                ...
                function (data) {
                    ...
                    angular.forEach($scope.miSaleConclusionRpt.data, function (item) {
                        if (item.totalConfirmedProformaAmount !== 0) {
                            params.data.push({ name: item.clientCountryNM, y: item.totalConfirmedProformaAmount });
                        }                        
                    });
                    jQuery('#miSaleConclusionRpt_PiConfirmedByCountry').highcharts({
                        chart: {
                        ...
                        title: {
                            text: 'ProformaInvoice Confirmed by country ' + $scope.miSaleConclusionRpt.param.season
                        },
                        tooltip: {
    ...
    $scope.miSaleConclusionRpt.method = {
        getData: function (season, successCallBack, errorCallBack) {
            console.log(miSaleConclusionRpt_PiConfirmedByCountry_Context.token);
            $.ajax({
                cache: false,
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + miSaleConclusionRpt_PiConfirmedByCountry_Context.token
                },
                type: "POST",
                dataType: 'json',
                url: miSaleConclusionRpt_PiConfirmedByCountry_Context.serviceUrl + 'get-pi-confirmed-by-country?season=' + season + '&saleID=-1',
                beforeSend: function () {
