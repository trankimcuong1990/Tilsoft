...
angular.module('tilsoftApp').controller('MiSaleConclusionRpt_PiByCountryController', ['$scope', '$rootScope', '$timeout', '$filter', function ($scope, $rootScope, $timeout, $filter) {
    $scope.miSaleConclusionRpt = {};
    //
    // property
    //
    $scope.miSaleConclusionRpt.data = null;
    $scope.miSaleConclusionRpt.param = miSaleConclusionRpt_PiByCountry_Context.param;
            ...
            $scope.miSaleConclusionRpt.method.getData(
                ...
                function (data) {
                    ...
                    angular.forEach($scope.miSaleConclusionRpt.data, function (item) {
                        if (item.totalProformaAmount !== 0) {
                            params.data.push({ name: item.clientCountryNM, y: item.totalProformaAmount });
                        }
                    });
                    jQuery('#miSaleConclusionRpt_PiByCountry').highcharts({
                        chart: {
                        ...
                        title: {
                            text: 'ProformaInvoice by country ' + $scope.miSaleConclusionRpt.param.season
                        },
                        tooltip: {
        ...
        getData: function (season, successCallBack, errorCallBack) {            
            $.ajax({
                ...
                headers: {
                    ...
                    'Authorization': 'Bearer ' + miSaleConclusionRpt_PiByCountry_Context.token
                },
                type: "POST",
                dataType: 'json',
                url: miSaleConclusionRpt_PiByCountry_Context.serviceUrl + 'get-pi-by-country?season=' + season + '&saleID=-1',
                beforeSend: function () {
