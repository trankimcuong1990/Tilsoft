...
tilsoftApp.controller('tilsoftController', ['$scope', '$timeout', '$cookieStore', 'dataService', function ($scope, $timeout, $cookieStore, dataService) {
    ...
    $scope.event = {
        ...
        reload: function () {
            ...
            $scope.pageIndex = 1;
            dataService.searchFilter.pageIndex = $scope.pageIndex;
            dataService.searchFilter.sortedBy = 'paymentNoteNo';
            $scope.event.search();
        },
        search: function (isLoadMore) {
            ...
            dataService.search(
                function (data) {
                    ...
                    $scope.supportdata.status = data.data.paymentSupportStatuses;
                    $scope.supportdata.types = data.data.paymentSupportTypes;
                    jQuery('#content').show();
