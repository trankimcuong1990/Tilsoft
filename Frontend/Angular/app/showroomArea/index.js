...
tilsoftApp.controller('tilsoftController', ['$scope', function ($scope) {
    ...
    $scope.event = {
        reload: function () {
            $scope.pageIndex = 1;
            jsonService.searchFilter.pageIndex = 1;
            jsonService.searchFilter.sortedBy = 'ShowroomAreaUD';
            $scope.event.search();
        },
        search: function () {
            ...
            jsonService.search(
                function (data) {
                    ...
                    angular.forEach($scope.data, function (item) {
                        $scope.generateBarcode_Code128(item.showroomAreaID, item.showroomAreaUD);
                    });
                },
                function (error) {
                    $scope.data = null;
                    ...
                    function (data) {
                        ...
                        if (data.message.type == 0) {
                            ...
                            for (var i = 0; i < $scope.data.length; i++) {
                                if ($scope.data[i].showroomAreaID == data.data) {
                                    j = i;
                                    break;
                                }
                            }
