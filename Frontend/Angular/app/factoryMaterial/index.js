...
tilsoftApp.controller('tilsoftController', ['$scope', function ($scope) {
    ...
    $scope.filters = {
        factoryMaterialUD: '',
        factoryMaterialNM: '',
        factoryMaterialEnglishName: '',
        factoryMaterialGroupID: null
    };
    $scope.pageIndex = 1;
    $scope.totalPage = 0;
    ...
    $scope.event = {
        reload: function () {
            $scope.pageIndex = 1;
            jsonService.searchFilter.pageIndex = 1;
            jsonService.searchFilter.sortedBy = 'factoryMaterialNM';
            $scope.event.search();
        },
        search: function () {
            ...
            jsonService.search(
                function (data) {
                    ...
                    $scope.factoryMaterialGroups = data.data.factoryMaterialGroups; console.log($scope.data);
                    ...
                    function (data) {
                        ...
                        if (data.message.type == 0) {
                            ...
                            for (var i = 0; i < $scope.data.length; i++) {
                                if ($scope.data[i].factoryMaterialID == data.data) {
                                    j = i;
                                    break;
                                }
                            }
        ...
        getExportExcel: function () {
            jsonService.printFactoryMaterialStock(
                function (data) {
                    jsHelper.processMessage(data.message);
                    if (data.message.type != 2) {
