...
tilsoftApp.controller('tilsoftController', ['$scope', '$timeout', 'dataService', function ($scope, $timeout, dataService) {
    ...
    $scope.data = null;
    $scope.remarkData = {
        sampleRemarkID: dataService.getIncrementId(),
    ...
    $scope.event = {
        init: function () {
            dataService.getInternalRemarkData(
                context.id,
                function (data) {
        ...
        add: function () {
            dataService.updateInternalRemarkData(
                    $scope.newRemarkData.sampleRemarkID,
                    $scope.newRemarkData,
                    function (data) {
        ...
        update: function (item) {
            dataService.updateInternalRemarkData(
                $scope.remarkData.sampleRemarkID,
                $scope.remarkData,
                function (data) {
                    jsHelper.processMessageEx(data.message);
                    if (data.message.type == 0) {
        ...
        delete: function (item) {
            if (confirm('Are you sure?')) {
                dataService.deleteInternalRemarkData(
                    item.sampleRemarkID,
                    function (data) {
                        jsHelper.processMessageEx(data.message);
                        if (data.message.type == 0) {
            ...
            fileUploader2.callback = function () {
                scope.$apply(function () {
                    scope.newRemarkData.remarkImageDTOs.push({
                        sampleRemarkImageID: dataService.getIncrementId(),
                        fileUD: '',
                        thumbnailLocation: '',
