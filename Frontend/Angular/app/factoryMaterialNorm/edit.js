...
tilsoftApp.controller('tilsoftController', ['$scope', function ($scope) {
    ...
    $scope.event = {
                ...
                jsonService.update(
                    ...
                    function (data) {
                        ...
                        if (data.message.type == 0) {
                            $scope.method.refresh(data.data.factoryMaterialNormID);
                        }
                    },
                    function (error) {
                        jsHelper.showMessage('warning', error);
        ...
        removeMaterial: function ($event, index) {
            $event.preventDefault();
            $scope.data.factoryMaterialNormDetails.splice(index, 1);
        }
    },
        ...
        event: {
            ...
            itemSelected: function ($event) {
                ...
                angular.forEach($scope.quickSearchMaterial.data, function (material_item) {
                    ...
                    {
                        if ($scope.data.factoryMaterialNormDetails.filter(function (o) { return o.factoryMaterialID == material_item.factoryMaterialID }).length == 0) // check item is existed in list
                        {
                            $scope.data.factoryMaterialNormDetails.push({
                                factoryMaterialNormDetailID: $scope.method.getNewID(),
