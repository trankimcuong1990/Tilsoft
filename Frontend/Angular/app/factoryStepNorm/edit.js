...
tilsoftApp.controller('tilsoftController', ['$scope', function ($scope) {
    ...
    $scope.event = {
                ...
                jsonService.update(
                    ...
                    function (data) {
                        ...
                        if (data.message.type == 0) {
                            $scope.method.refresh(data.data.factoryStepNormID);
                        }
                    },
                    function (error) {
                        jsHelper.showMessage('warning', error);
        ...
        addStep: function ($event) {
            $event.preventDefault();
            $scope.data.factoryStepNormDetails.push({
                factoryStepNormDetailID: $scope.method.getNewID(),
            });
        },
        removeStep: function ($event, index) {
            $event.preventDefault();
            $scope.data.factoryStepNormDetails.splice(index, 1);
        }
    },
