        ...
        public object CustomFunction(int userId, string identifier, System.Collections.Hashtable input, out Library.DTO.Notification notification)
        {
            switch (identifier.ToLower())
            {
                case "getpreviewdata":
                    ...
                    {
                        notification = new Library.DTO.Notification() { Type = Library.DTO.NotificationType.Error, Message = "Unknow quotation id" };
                        return null;
                    }
                    return bll.GetPreviewData(userId, Convert.ToInt32(input["id"]), out notification);
