...
namespace Module.FactoryProformaInvoiceOverviewRpt
{
        ...
        public object CustomFunction(int userId, string identifier, System.Collections.Hashtable input, out Library.DTO.Notification notification)
        {
            switch (identifier.ToLower())
            {
                case "getexcelreport":
                    ...
                    {
                        throw new Exception("Unknow supplier");
                    }
                    if (!input.ContainsKey("season"))
                    {
                        throw new Exception("Unknow season");
                    }
                    return bll.GetExcelReportData(userId, Convert.ToInt32(input["supplierId"].ToString()), input["season"].ToString(), out notification);
            }
