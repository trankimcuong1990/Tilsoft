//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DAL.BackSaleOrderMng
{
    using System;
    using System.Collections.Generic;
    
    public partial class SaleOrderDetailSparepart
    {
        public SaleOrderDetailSparepart()
        {
            this.BackSaleOrderDetailSparepart = new HashSet<BackOrderDetail>();
            this.OriginSaleOrderDetailSparepart = new HashSet<BackOrderDetail>();
        }
    
        public int SaleOrderDetailSparepartID { get; set; }
        public Nullable<int> OrderQnt { get; set; }
        public Nullable<decimal> UnitPrice { get; set; }
        public Nullable<int> ProductStatusID { get; set; }
        public Nullable<int> V4ID { get; set; }
        public string ClientArticleCode { get; set; }
        public string ClientArticleName { get; set; }
        public string ClientEANCode { get; set; }
    
        public virtual OfferLineSparepart OfferLineSparepart { get; set; }
        public virtual ICollection<BackOrderDetail> BackSaleOrderDetailSparepart { get; set; }
        public virtual ICollection<BackOrderDetail> OriginSaleOrderDetailSparepart { get; set; }
        public virtual SaleOrder SaleOrder { get; set; }
    }
}
