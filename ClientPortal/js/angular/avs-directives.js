...
// scroller
angular.module('avs-directives').directive('avsSimpleTable', function ($timeout) {
    return {
        restrict: 'A',
        scope: {
            handler: '='
        ...
        link: function (scope, elem, attrs) {
            ...
            if (scope.handler) {
                ...
                var jHeader = obj.find('thead tr th');
                var loadMoreRow = '<tr>';
                loadMoreRow += '<td colspan="' + jHeader.length + '">';
                loadMoreRow += '<a href="javascript:void(0)" class="btn btn-default avs-load-more"><i class="fa fa-database"></i> Load more</a>';
                loadMoreRow += '</td>';
                loadMoreRow += '</tr>';
                obj.find('tbody').append(loadMoreRow);

                // add load more function
                obj.find('.avs-load-more').each(function () {
                ...
                obj.find('thead tr th').each(function () {
                    if (angular.element(this).hasClass('sorting') || angular.element(this).hasClass('sorting_asc') || angular.element(this).hasClass('sorting_desc')) {
                        angular.element(this).on('click', function () {
                            ...
                            angular.element(this).parent().find('th').each(function () {
                                if (angular.element(this).hasClass('sorting') || angular.element(this).hasClass('sorting_asc') || angular.element(this).hasClass('sorting_desc')) {
    ...
    return {
        ...
        link: function (scope, elem, attrs) {
            ...
            $timeout(function () {
                ...
                obj.find('.avs-table-body table').css('width', (totalWidth + 1) + 'px');
